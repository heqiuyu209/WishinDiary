...
CREATE TABLE IF NOT EXISTS daily_logs (
    ...
    -- 006: 新增自记录维度字段（与 006_add_daily_log_fields.sql / 0003 迁移对齐）
    sleep_duration_minutes INT NOT NULL DEFAULT 0 COMMENT '睡眠时长（分钟），0=未记录，取值范围 0~1440',
    sleep_quality INT NOT NULL DEFAULT 0 COMMENT '睡眠质量：0=未填/很差 1=差 2=一般 3=好',
    is_late_night TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否熬夜：0=否 1=是',
    is_medication TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否用药：0=否 1=是',
    medication_note VARCHAR(100) DEFAULT NULL COMMENT '用药说明（≤100字），未用药时可为空',
    symptom_levels JSON NOT NULL DEFAULT (JSON_OBJECT('headache',0,'bloat',0,'breast_tenderness',0,'fatigue',0)) COMMENT '症状明细：headache/bloat/breast_tenderness/fatigue 各取 0=无 1=轻 2=中 3=重（应用层校验）',
    ...
    CHECK (sleep_quality BETWEEN 0 AND 3),
    CHECK (JSON_VALID(symptom_levels)),
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
